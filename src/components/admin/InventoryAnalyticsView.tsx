...
export function InventoryAnalyticsView({ data }: { data: InventoryAnalytics }) {
  const kpis = [
    ...
    { label: 'Dead Stock (90d)', value: data.deadStockCount, icon: Package, color: 'text-foreground/' },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6">
          <h2 className="text-foreground font-semibold mb-6">Restock Recommendations</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="text-left text-foreground/ text-xs border-b border-foreground/">
              ...
              <tbody className="divide-y divide-white/5">
                {data.restockRecommendations.map(r => (
                  <tr key={r.id}>
                    <td className="py-3 text-sm text-foreground">{r.name}</td>
                    <td className="py-3 text-sm text-amber-400 font-semibold text-right">{r.stock}</td>
                    <td className="py-3 text-sm text-foreground/ text-right">{r.velocity30d} units</td>
                ...
                {data.restockRecommendations.length === 0 && (
                  <tr>
                    <td colSpan={4} className="py-8 text-center text-foreground/ text-sm">Inventory looks healthy! No immediate restocks needed.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="glass-card p-6">
          <h2 className="text-foreground font-semibold mb-6">Inventory Value by Category</h2>
                ...
                {data.categoryDistribution.map((c, i) => (
                  <div key={c.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-foreground/ text-sm truncate max-w-[120px]">{c.name}</span>
                    </div>
                    <span className="text-foreground font-medium text-sm">{formatCurrency(c.value)}</span>
                  </div>
                ))}
              </div>
            </div>
          ) : (
             <p className="py-8 text-center text-foreground/ text-sm">No category data available.</p>
          )}
        </div>
      </div>
    </div>
