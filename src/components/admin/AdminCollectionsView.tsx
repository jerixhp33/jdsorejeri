...
interface AdminCollectionsViewProps {
  collections: any[];
  allProducts: Array<{ id: string; name: string; product_type: string; slug: string; images?: { url: string }[] }>;
}
                  ...
                  {filteredProducts.map(product => (
                    <label key={product.id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 cursor-pointer">
                      ...
                      {product.images?.[0]?.url ? (
                        /* eslint-disable-next-line @next/next/no-img-element */
                        <img src={product.images[0].url} alt="" className="w-8 h-8 object-cover rounded-md shrink-0 bg-white/10" />
                      ) : (
