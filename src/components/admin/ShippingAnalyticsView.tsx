      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {kpis.map((k) => (
          <div key={k.label} className="glass-card p-5">
            ...
            <p className="font-display text-lg lg:text-2xl font-bold text-foreground mb-1">{k.value}</p>
            <p className="text-foreground/ text-[10px] lg:text-xs">{k.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6">
          ...
          {data.courierPerformance.length > 0 ? (
            ...
            <p className="py-12 text-center text-foreground/ text-sm">No courier shipment data available yet.</p>
          )}
        </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-white/5">
                {data.courierPerformance.map((c: any) => (
                  <tr key={c.name}>
                    <td className="py-4 text-sm text-foreground capitalize">{c.name}</td>
                    <td className="py-4 text-sm text-foreground/ text-right">{c.total}</td>
                ...
                {data.courierPerformance.length === 0 && (
                  <tr>
                    <td colSpan={3} className="py-8 text-center text-foreground/ text-sm">No courier shipments made.</td>
                  </tr>
                )}
              </tbody>
            </table>
