  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="glass-card p-6">
        ...
        {/* Custom Poster Pricing & Specifications Panel */}
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-8 space-y-4">
          <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wider flex items-center gap-2">
            🎨 Custom Photo Poster Prices & Specifications
          </h3>
          <p className="text-xs text-white/50">Control size prices, frame add-on costs, paper GSM, and dispatch specifications live on your store.</p>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">A5 Size Price (₹)</label>
              <input
                type="number"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_price_a5') || { value: 199 } as any)}
                onBlur={(e) => updateSetting('custom_poster_price_a5', e.target.value)}
                className="input-luxe text-sm font-mono"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">A4 Size Price (₹)</label>
              <input
                type="number"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_price_a4') || { value: 299 } as any)}
                onBlur={(e) => updateSetting('custom_poster_price_a4', e.target.value)}
                className="input-luxe text-sm font-mono"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">A3 Size Price (₹)</label>
              <input
                type="number"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_price_a3') || { value: 449 } as any)}
                onBlur={(e) => updateSetting('custom_poster_price_a3', e.target.value)}
                className="input-luxe text-sm font-mono"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">Black Frame Add-on (₹)</label>
              <input
                type="number"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_frame_black_price') || { value: 150 } as any)}
                onBlur={(e) => updateSetting('custom_poster_frame_black_price', e.target.value)}
                className="input-luxe text-sm font-mono"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">White Frame Add-on (₹)</label>
              <input
                type="number"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_frame_white_price') || { value: 150 } as any)}
                onBlur={(e) => updateSetting('custom_poster_frame_white_price', e.target.value)}
                className="input-luxe text-sm font-mono"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">Wood Frame Add-on (₹)</label>
              <input
                type="number"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_frame_wood_price') || { value: 200 } as any)}
                onBlur={(e) => updateSetting('custom_poster_frame_wood_price', e.target.value)}
                className="input-luxe text-sm font-mono"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">Paper Specification</label>
              <input
                type="text"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_paper_gsm') || { value: '300 GSM Gallery Paper' } as any)}
                onBlur={(e) => updateSetting('custom_poster_paper_gsm', e.target.value)}
                className="input-luxe text-sm"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">Paper Finish</label>
              <input
                type="text"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_paper_finish') || { value: 'Ultra-Matte Archival Finish' } as any)}
                onBlur={(e) => updateSetting('custom_poster_paper_finish', e.target.value)}
                className="input-luxe text-sm"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">Dispatch Time SLA</label>
              <input
                type="text"
                defaultValue={getValue(settings.find(s => s.key === 'custom_poster_dispatch_time') || { value: 'Dispatched in 24h' } as any)}
                onBlur={(e) => updateSetting('custom_poster_dispatch_time', e.target.value)}
                className="input-luxe text-sm"
              />
            </div>
          </div>
