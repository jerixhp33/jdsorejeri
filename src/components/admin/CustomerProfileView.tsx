    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border-l-4 border-l-luxe-accent">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-foreground/">
            ...
            <p className="text-foreground/ text-xs mt-2 font-mono">Customer ID: {customer.customer_number}</p>
          </div>
        </div>

        {/* Loyalty Quick Stats */}
        <div className="flex items-center gap-6 p-4 rounded-xl bg-black/40 border border-foreground/ min-w-[250px]">
          <div>
          ...
          <div>
            <p className="text-foreground/ text-[10px] uppercase tracking-wider mb-0.5">Membership Tier</p>
            <p className="text-foreground font-bold capitalize text-lg">{customer.membership_tier}</p>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              <h2 className="text-lg font-bold text-foreground">Analytics Overview</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="glass-card p-5 rounded-xl">
                  <TrendingUp className="w-5 h-5 text-luxe-accent mb-3" />
                  <p className="text-foreground/ text-xs uppercase tracking-wider mb-1">Lifetime Value</p>
                  <p className="text-2xl font-bold text-foreground">{formatCurrency(analytics.lifetimeValue)}</p>
                </div>
                <div className="glass-card p-5 rounded-xl">
                  <Package className="w-5 h-5 text-blue-400 mb-3" />
                  <p className="text-foreground/ text-xs uppercase tracking-wider mb-1">Total Orders</p>
                  <p className="text-2xl font-bold text-foreground">{analytics.totalOrders}</p>
                </div>
                <div className="glass-card p-5 rounded-xl">
                  <Activity className="w-5 h-5 text-purple-400 mb-3" />
                  <p className="text-foreground/ text-xs uppercase tracking-wider mb-1">Avg Order Value</p>
                  <p className="text-2xl font-bold text-foreground">{formatCurrency(analytics.averageOrderValue)}</p>
                </div>
                <div className="glass-card p-5 rounded-xl bg-red-500/5 border-red-500/10">
                  <AlertTriangle className="w-5 h-5 text-red-400 mb-3" />
                  <p className="text-foreground/ text-xs uppercase tracking-wider mb-1">Return Rate</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="glass-card p-6">
                  <h3 className="text-sm font-semibold text-foreground/ uppercase tracking-wider mb-4">Preferences</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center py-2 border-b border-foreground/">
                      <span className="text-foreground/ text-sm">Favorite Category</span>
                      <span className="text-foreground font-medium capitalize">{analytics.favoriteCategory || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-foreground/">
                      <span className="text-foreground/ text-sm">Preferred Payment</span>
                      <span className="text-foreground font-medium capitalize">{analytics.favoritePaymentMethod || 'N/A'}</span>
                    </div>
                  </div>
                </div>
                
                <div className="glass-card p-6">
                   <h3 className="text-sm font-semibold text-foreground/ uppercase tracking-wider mb-4">Quick Notes</h3>
                   {customer.admin_notes ? (
                     <p className="text-foreground/ text-sm leading-relaxed whitespace-pre-wrap">{customer.admin_notes}</p>
                   ) : (
                     <p className="text-foreground/ text-sm italic">No admin notes added for this customer.</p>
                   )}
          ...
          {activeTab === 'timeline' && (
            <div className="glass-card p-6 md:p-8">
              <h2 className="text-lg font-bold text-foreground mb-8">Customer Journey</h2>
              <div className="space-y-8 relative before:absolute before:inset-0 before:ml-[19px] before:-translate-x-px before:h-full before:w-0.5 before:bg-foreground/">
                {timeline.map((event, i) => (
                  <div key={i} className="relative flex items-start gap-6">
                    <div className={cn("relative z-10 w-10 h-10 rounded-full flex items-center justify-center shrink-0 border border-foreground/ shadow-xl", event.bg)}>
                    ...
                    <div className="flex-1 pb-2">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1">
                        ...
                        <span className="text-foreground/ text-xs">{formatDate(event.date)}</span>
                      </div>
          ...
          {activeTab === 'notes' && (
            <div className="glass-card p-6">
              ...
              <p className="text-foreground/ text-sm mb-6">These notes are strictly for internal staff and are never visible to the customer.</p>
          ...
          {['orders', 'addresses'].includes(activeTab) && (
            <div className="glass-card p-12 text-center">
              <p className="text-foreground/ text-sm">Detailed views for {activeTab} are actively tracking from the Orders module.</p>
            </div>
          )}
        </div>
      </div>
