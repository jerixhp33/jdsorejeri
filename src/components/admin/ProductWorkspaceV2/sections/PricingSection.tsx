  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-white/50 text-xs uppercase tracking-wide mb-2 block">Selling Price (₹) *</label>
          <input 
            type="number"
        ...
        <div>
          <label className="text-white/50 text-xs uppercase tracking-wide mb-2 block">Original Price (₹)</label>
          <input 
            type="number"
            value={formData.original_price}
            onChange={(e) => updateField('original_price', Number(e.target.value))}
            className="input-luxe w-full text-white/50 line-through"
            placeholder="0.00"
          />
        </div>
      </div>
      ...
      <div className="border-t border-white/5 pt-6 grid grid-cols-2 gap-4">
        <div>
          <label className="text-white/50 text-xs uppercase tracking-wide mb-2 block">Cost Price (₹)</label>
          <input 
            type="number"
        ...
        <div className="bg-luxe-accent/5 rounded-xl border border-luxe-accent/20 p-4 flex flex-col justify-center">
          <div className="flex justify-between items-center mb-2">
            <span className="text-white/50 text-xs">Profit</span>
            <span className="text-luxe-accent font-medium">{formatCurrency(profit > 0 ? profit : 0)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-white/50 text-xs">Margin</span>
