    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">Manage users waiting for out-of-stock products</p>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full sm:w-64 bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-2 text-sm text-white focus:outline-none focus:border-luxe-accent transition-colors"
          />
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-white/10 bg-white/[0.02]">
                <th className="p-4 font-semibold text-white/70">Product</th>
                <th className="p-4 font-semibold text-white/70">Variant</th>
                <th className="p-4 font-semibold text-white/70">Customer</th>
                <th className="p-4 font-semibold text-white/70">Date Requested</th>
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-luxe-dark overflow-hidden flex-shrink-0">
                          {w.product?.images?.[0]?.url && (
                            <img src={w.product.images[0].url} alt="" className="w-full h-full object-cover" />
                          )}
                        </div>
                        <div>
                          <p className="text-white font-medium">{w.product?.name || 'Unknown Product'}</p>
                          <p className="text-white/40 text-[10px] uppercase">{w.product?.product_type}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <p className="text-white">{w.user_profile?.name || 'Unknown User'}</p>
                      <a href={`mailto:${w.user_profile?.email}`} className="text-white/40 text-xs hover:text-luxe-accent flex items-center gap-1 mt-0.5">
