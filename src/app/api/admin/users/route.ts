...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin.from('user_profiles').insert(body).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function PATCH(req: NextRequest) {
  ...
  const col = id ? 'id' : 'key';
  const val = id ?? key;
  const { data, error } = await admin.from('user_profiles').update(updates).eq(col, val).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await admin.from('user_profiles').delete().eq('id', body.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true });
}
