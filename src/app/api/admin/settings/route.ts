...
export async function GET() {
  ...
  const { data, error } = await admin.from('settings').select('*').order('created_at', { ascending: false });
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin.from('settings').insert(body).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await admin.from('settings').delete().eq('id', body.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true });
}
