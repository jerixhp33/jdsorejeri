...
export async function GET() {
  ...
  const { data, error } = await admin.from('product_categories').select('*').order('created_at', { ascending: false });
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin.from('product_categories').insert(body).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function PATCH(req: NextRequest) {
  ...
  const col = id ? 'id' : 'key';
  const val = id ?? key;
  const { data, error } = await admin.from('product_categories').update(updates).eq(col, val).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await admin.from('product_categories').delete().eq('id', body.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true });
}
