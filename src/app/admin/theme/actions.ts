...
export async function upsertHomeTheme(data: Partial<HomeThemeConfig>) {
  try {
    ...
    const payload: any = {
      ...
      glow_secondary_color: data.glow_secondary_color || 'rgba(79, 172, 254, 0.55)',
      glow_tertiary_color: data.glow_tertiary_color || 'rgba(240, 147, 251, 0.55)',
      glow_quaternary_color: data.glow_quaternary_color || 'rgba(0, 255, 135, 0.55)',
      text_accent_color: data.text_accent_color || '#c8a96e',
      element_image_url: data.element_image_url || (data.element_images?.[0] || null),
      element_images: data.element_images || (data.element_image_url ? [data.element_image_url] : []),
