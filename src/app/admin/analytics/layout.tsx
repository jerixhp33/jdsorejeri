  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold text-foreground">Business Analytics</h1>
          <p className="text-foreground/ text-sm mt-1">Deep insights into your store's performance</p>
        </div>
      ...
      {/* Tabs Navigation */}
      <div className="flex items-center gap-1 overflow-x-auto pb-2 border-b border-foreground/ no-scrollbar">
        {tabs.map((tab) => {
            ...
            <Link
              ...
              className={cn(
                ...
                isActive
                  ...
                  : "border-transparent text-foreground/ hover:text-foreground hover:bg-foreground/ rounded-t-lg"
              )}
            >
      ...
      {isAiModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-luxe-darker border border-foreground/ rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between p-6 border-b border-foreground/ bg-black/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-foreground">AI Business Assistant</h2>
                  <p className="text-foreground/ text-xs">Powered by Groq 8B</p>
                </div>
              </div>
              <button onClick={() => setIsAiModalOpen(false)} className="text-foreground/ hover:text-foreground transition-colors">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {isGenerating ? (
                <div className="flex flex-col items-center justify-center py-12 text-foreground/ space-y-4">
                ...
                <div className="prose prose-invert prose-p:text-foreground/ prose-headings:text-foreground prose-li:text-foreground/ prose-a:text-luxe-accent max-w-none prose-sm sm:prose-base">
                  {aiInsights?.split('\n').map((line, i) => {
            ...
            <div className="p-4 border-t border-foreground/ bg-black/20 text-center">
               <p className="text-[10px] text-foreground/ uppercase tracking-widest">AI generated insights may occasionally contain inaccuracies.</p>
            </div>
          </div>
        </div>
      )}
